  ...
  return (
    <div className="flex-1 space-y-6 p-8 pt-6">
      ...
      <Card className="mt-6"><CardHeader><CardTitle className="flex items-center gap-2"><ListChecks className="h-5 w-5"/>Propostas Geradas</CardTitle></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>ID</TableHead><TableHead>Cliente</TableHead><TableHead>Data</TableHead><TableHead>Investimento</TableHead><TableHead>Mensal</TableHead><TableHead className="text-right">Ações</TableHead></TableRow></TableHeader><TableBody>{proposals.length === 0 ? <TableRow><TableCell colSpan={6} className="text-center h-20 text-muted-foreground">Vazio</TableCell></TableRow> : proposals.map(p => ( <TableRow key={p.id}><TableCell className="font-bold">#{p.id}</TableCell><TableCell>{p.clientName}</TableCell><TableCell>{format(parseISO(p.proposalDate), 'dd/MM/yyyy')}</TableCell><TableCell className="text-primary font-medium">{p.totalOneTime.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</TableCell><TableCell className="text-emerald-500 font-medium">{p.totalMonthly.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</TableCell><TableCell className="text-right"><div className="flex justify-end gap-1"><Button variant="outline" size="icon" className="h-8 w-8 text-muted-foreground" onClick={() => handleCloneProposalClick(p)} title="Clonar Proposta"><Copy className="h-4 w-4" /></Button><Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleEditProposalClick(p)} title="Editar"><Pencil className="h-4 w-4" /></Button><Button variant="outline" size="icon" className="h-8 w-8" onClick={() => setSelectedProposal(p)} title="Imprimir"><Printer className="h-4 w-4" /></Button><Button variant="outline" size="icon" className="h-8 w-8 text-destructive" onClick={() => setDeletingProposal(p)} title="Excluir"><Trash2 className="h-4 w-4" /></Button></div></TableCell></TableRow> ))}</TableBody></Table></CardContent></Card>
        <Dialog open={!!selectedProposal} onOpenChange={(open) => !open && setSelectedProposal(null)}><DialogContent className="sm:max-w-5xl h-[90vh] flex flex-col"><DialogHeader className="print-hide"><DialogTitle>Pré-visualização da Proposta</DialogTitle></DialogHeader>{selectedProposal && ( <ScrollArea className="flex-1 -mx-6 px-6"><div id="proposal-preview" className="bg-white text-black p-24 shadow-lg max-w-4xl mx-auto my-8 border-none" style={{ fontFamily: "'Arial', sans-serif", letterSpacing: '0.01em' }}><div className="flex justify-between items-center mb-16" style={{ minHeight: '120px' }}><div className="flex items-center gap-10">{companyProfile.logoUrl && <img src={companyProfile.logoUrl} alt="Logo" className="max-h-24 w-auto" />}<div className="border-l-2 border-gray-100 pl-8"><h1 className="text-2xl font-bold uppercase tracking-tighter leading-tight" style={{ margin: '0' }}>{companyProfile.name}</h1></div></div><div className="text-right text-[11px] leading-relaxed text-gray-400 font-medium"><p>{companyProfile.address}</p><p>{companyProfile.email}</p><p className="text-gray-800 font-bold mt-1 text-sm">{formatPhoneNumber(companyProfile.phone)}</p></div></div><div className="text-center mb-24"><h2 className="text-3xl font-bold uppercase tracking-[0.3em] border-b-2 border-black pb-6 inline-block">Proposta Comercial</h2></div><div className="grid grid-cols-2 gap-20 mb-20 text-sm"><div className="space-y-2 text-left"><p className="font-bold text-gray-300 uppercase text-[10px] tracking-[0.2em] mb-3">Destinatário</p><p className="font-bold text-2xl leading-none tracking-tight mb-2">{selectedProposal.clientName}</p>{selectedProposal.contactName && ( <p className="text-lg font-bold text-primary uppercase bg-primary/5 px-2 py-1 rounded inline-block">A/C: {selectedProposal.contactName}</p> )}<p className="text-gray-500 font-medium mt-1">{formatPhoneNumber(selectedProposal.clientPhone || '')}</p></div><div className="text-right flex flex-col justify-end space-y-3"><p className="text-gray-300 uppercase text-[10px] tracking-[0.2em]">Informações Gerais</p><p className="leading-none text-base"><span className="font-bold">Nº Proposta:</span> {selectedProposal.id}</p><p className="leading-none text-base"><span className="font-bold">Data Emissão:</span> {format(parseISO(selectedProposal.proposalDate), 'dd/MM/yyyy')}</p><p className="leading-none text-base text-destructive font-bold"><span className="text-black">Validade:</span> {format(parseISO(selectedProposal.validityDate), 'dd/MM/yyyy')}</p></div></div><div className="space-y-8 text-sm mb-20 leading-relaxed text-justify text-gray-700 font-medium px-4"><p>Temos a satisfação de apresentar nossa proposta comercial, desenvolvida com foco total na excelência tecnológica e na eficiência operacional que sua empresa demanda.</p><p>Com ampla experiência de mercado, a {companyProfile.name} combina consultoria especializada e as mais modernas ferramentas de TI para entregar soluções ágeis, seguras e personalizadas. Nosso compromisso é com a qualidade absoluta, desde o primeiro contato até o suporte contínuo.</p></div><div className="mb-20"><table className="w-full text-left text-sm border-collapse"><thead className="bg-gray-50"><tr><th className="p-5 font-bold border-b-2 border-gray-900 uppercase text-[10px] tracking-widest">Item / Descrição</th><th className="p-5 text-center font-bold border-b-2 border-gray-900 uppercase text-[10px] tracking-widest">Qtd.</th><th className="p-5 text-right font-bold border-b-2 border-gray-900 uppercase text-[10px] tracking-widest">Unitário</th><th className="p-5 text-right font-bold border-b-2 border-gray-900 uppercase text-[10px] tracking-widest">Subtotal</th></tr></thead><tbody>{selectedProposal.items.map((it, i) => ( <tr key={i} className="border-b border-gray-100 hover:bg-gray-50/30 transition-colors"><td className="p-5 font-bold text-gray-800">{it.name} {it.isMonthly && <span className="text-[9px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full ml-3 tracking-widest">MENSAL</span>}</td><td className="p-5 text-center font-medium">{it.quantity}</td><td className="p-5 text-right text-gray-500 font-medium">{(Number(it.price) || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td><td className="p-5 text-right font-bold text-gray-900">{(it.quantity * it.price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td></tr> ))}</tbody></table><div className="flex justify-end mt-12"><div className="w-1/2 space-y-4"><div className="flex justify-between items-center text-sm py-3 border-b border-gray-100"><span className="font-bold text-gray-400 uppercase tracking-[0.2em] text-[10px]">Investimento Total:</span> <span className="font-bold text-2xl text-gray-900">{selectedProposal.totalOneTime.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div><div className="flex justify-between items-center text-sm py-3 bg-emerald-50/50 px-3 rounded-lg text-emerald-800"><span className="font-bold uppercase tracking-[0.2em] text-[10px]">Recorrência Mensal:</span> <span className="font-bold text-2xl">{selectedProposal.totalMonthly.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div></div></div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-24"><div className="bg-gray-50/80 p-10 rounded-3xl text-sm border border-gray-100 h-full"><h3 className="font-bold mb-8 uppercase text-[10px] text-gray-300 tracking-[0.3em]">Condições de Pagamento</h3><div className="space-y-6"><div className="flex items-center justify-between"><span className="font-bold text-gray-400 text-[10px] uppercase tracking-wider">Forma:</span> <span className="font-bold uppercase text-base text-gray-800">{selectedProposal.paymentMethod}</span></div><div className="flex flex-col gap-2 pt-4 border-t border-white"><div className="flex items-center justify-between"><span className="font-bold text-gray-400 text-[10px] uppercase tracking-wider">Parcelamento:</span> <span className="font-bold text-lg text-primary">{selectedProposal.installments}x de {(selectedProposal.totalOneTime / selectedProposal.installments).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div>{selectedProposal.firstAsDownPayment && selectedProposal.installments > 1 && ( <p className="text-[10px] font-bold text-primary text-right uppercase tracking-widest mt-1">(1ª Parcela como entrada)</p> )}</div></div></div>{selectedProposal.observations && ( <div className="bg-gray-50/80 p-10 rounded-3xl text-sm border border-gray-100 h-full"><h3 className="font-bold mb-8 uppercase text-[10px] text-gray-300 tracking-[0.3em]">Observações Adicionais</h3><p className="whitespace-pre-wrap italic text-gray-600 leading-relaxed text-[11px] text-justify font-medium">{selectedProposal.observations}</p></div> )}</div><div className="mt-40 text-center"><div className="w-48 h-px bg-gray-200 mx-auto mb-6"></div><p className="text-[11px] text-gray-300 font-bold uppercase tracking-[0.4em]">{companyProfile.name}</p></div></div></ScrollArea> )}<DialogFooter className="print-hide border-t pt-4 flex-wrap gap-2"><Button variant="outline" onClick={() => setSelectedProposal(null)}>Fechar</Button><Button variant="secondary" onClick={handleDownloadPdf} disabled={isDownloading}>{isDownloading ? <Loader2 className="animate-spin h-4 w-4" /> : <Download className="h-4 w-4" />} PDF</Button><Button variant="outline" className="gap-2" onClick={() => handleSendWhatsAppText(selectedProposal!)} title="Enviar resumo como texto"><Send className="h-4 w-4" /> WhatsApp (Texto)</Button><Button className="gap-2 bg-emerald-600 hover:bg-emerald-700" onClick={() => handleSharePdf(selectedProposal!)} disabled={isSharing}>{isSharing ? <Loader2 className="animate-spin h-4 w-4" /> : <Share2 className="h-4 w-4" />} Enviar PDF (WhatsApp)</Button></DialogFooter></DialogContent></Dialog>
