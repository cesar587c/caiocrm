  ...
  return (
    <div className="flex-1 space-y-6 p-8 pt-6">
      ...
      <Card className="mt-6"><CardHeader><CardTitle className="flex items-center gap-2"><ListChecks className="h-5 w-5"/>Propostas Geradas</CardTitle></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>ID</TableHead><TableHead>Cliente</TableHead><TableHead>Data</TableHead><TableHead>Investimento</TableHead><TableHead>Mensal</TableHead><TableHead className="text-right">Ações</TableHead></TableRow></TableHeader><TableBody>{proposals.length === 0 ? <TableRow><TableCell colSpan={6} className="text-center h-20 text-muted-foreground">Vazio</TableCell></TableRow> : proposals.map(p => ( <TableRow key={p.id}><TableCell className="font-bold">#{p.id}</TableCell><TableCell>{p.clientName}</TableCell><TableCell>{format(parseISO(p.proposalDate), 'dd/MM/yyyy')}</TableCell><TableCell className="text-primary font-medium">{p.totalOneTime.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</TableCell><TableCell className="text-emerald-500 font-medium">{p.totalMonthly.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</TableCell><TableCell className="text-right"><div className="flex justify-end gap-1"><Button variant="outline" size="icon" className="h-8 w-8 text-muted-foreground" onClick={() => handleCloneProposalClick(p)} title="Clonar Proposta"><Copy className="h-4 w-4" /></Button><Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleEditProposalClick(p)} title="Editar"><Pencil className="h-4 w-4" /></Button><Button variant="outline" size="icon" className="h-8 w-8" onClick={() => setSelectedProposal(p)} title="Imprimir"><Printer className="h-4 w-4" /></Button><Button variant="outline" size="icon" className="h-8 w-8 text-destructive" onClick={() => setDeletingProposal(p)} title="Excluir"><Trash2 className="h-4 w-4" /></Button></div></TableCell></TableRow> ))}</TableBody></Table></CardContent></Card>
        <Dialog open={!!selectedProposal} onOpenChange={(open) => !open && setSelectedProposal(null)}><DialogContent className="sm:max-w-[1000px] h-[95vh] flex flex-col p-0"><DialogHeader className="print-hide p-6 pb-0"><DialogTitle>Pré-visualização da Proposta (Formato A4)</DialogTitle></DialogHeader>{selectedProposal && ( <ScrollArea className="flex-1 bg-muted/30 p-8"><div id="proposal-preview" className="bg-white text-black mx-auto border-none overflow-hidden" style={{ width: '210mm', minHeight: '297mm', padding: '25mm', fontFamily: "Arial, sans-serif", boxSizing: 'border-box', letterSpacing: 'normal', wordSpacing: 'normal', fontVariantLigatures: 'none', lineHeight: '1.2' }}><div className="flex justify-between items-center mb-12" style={{ borderBottom: '1px solid #eee', paddingBottom: '8mm' }}><div className="flex items-center gap-8">{companyProfile.logoUrl && <img src={companyProfile.logoUrl} alt="Logo" className="max-h-20 w-auto" />}<div style={{ letterSpacing: 'normal' }}><h1 className="text-xl font-bold uppercase m-0" style={{ letterSpacing: 'normal', wordSpacing: 'normal' }}>{companyProfile.name}</h1><p className="text-[10px] text-gray-500 font-medium" style={{ letterSpacing: 'normal' }}>{companyProfile.email} | {formatPhoneNumber(companyProfile.phone)}</p></div></div><div className="text-right text-[10px] text-gray-400 font-medium leading-relaxed max-w-[200px]" style={{ letterSpacing: 'normal' }}><p>{companyProfile.address}</p></div></div><div className="text-center mb-12" style={{ letterSpacing: 'normal' }}><h2 className="text-2xl font-bold uppercase border-b-2 border-black pb-4 inline-block" style={{ letterSpacing: 'normal', wordSpacing: 'normal' }}>Proposta Comercial</h2></div><div className="grid grid-cols-2 gap-12 mb-12 text-sm"><div className="space-y-1"><p className="font-bold text-gray-400 uppercase text-[9px] mb-2" style={{ letterSpacing: 'normal' }}>Destinatário</p><p className="font-bold text-xl leading-tight mb-1" style={{ letterSpacing: 'normal', wordSpacing: 'normal' }}>{selectedProposal.clientName}</p>{selectedProposal.contactName && ( <p className="text-base font-bold text-primary uppercase" style={{ letterSpacing: 'normal' }}>A/C: {selectedProposal.contactName}</p> )}<p className="text-gray-500 font-medium" style={{ letterSpacing: 'normal' }}>{formatPhoneNumber(selectedProposal.clientPhone || '')}</p></div><div className="text-right space-y-1 self-end" style={{ letterSpacing: 'normal' }}><p className="leading-none text-xs"><span className="font-bold">Nº Proposta:</span> {selectedProposal.id}</p><p className="leading-none text-xs"><span className="font-bold">Emissão:</span> {format(parseISO(selectedProposal.proposalDate), 'dd/MM/yyyy')}</p><p className="leading-none text-xs text-red-600 font-bold"><span className="text-black">Validade:</span> {format(parseISO(selectedProposal.validityDate), 'dd/MM/yyyy')}</p></div></div><div className="space-y-4 text-[12px] mb-12 leading-relaxed text-left text-gray-700" style={{ letterSpacing: 'normal', wordSpacing: 'normal' }}><p>Temos a satisfação de apresentar nossa proposta comercial, desenvolvida com foco total na excelência tecnológica e na eficiência operacional que sua empresa demanda.</p><p>Com ampla experiência de mercado, a {companyProfile.name} combina consultoria especializada e as mais modernas ferramentas de TI para entregar soluções ágeis, seguras e personalizadas. Nosso compromisso é com a qualidade absoluta, desde o primeiro contato até o suporte contínuo.</p></div><div className="mb-12"><table className="w-full text-left text-[11px] border-collapse" style={{ letterSpacing: 'normal' }}><thead className="bg-gray-50"><tr><th className="p-3 font-bold border-b border-gray-900 uppercase text-[9px]">Item / Descrição</th><th className="p-3 text-center font-bold border-b border-gray-900 uppercase text-[9px]">Qtd.</th><th className="p-3 text-right font-bold border-b border-gray-900 uppercase text-[9px]">Unitário</th><th className="p-3 text-right font-bold border-b border-gray-900 uppercase text-[9px]">Subtotal</th></tr></thead><tbody>{selectedProposal.items.map((it, i) => ( <tr key={i} className="border-b border-gray-100"><td className="p-3 font-bold text-gray-800" style={{ letterSpacing: 'normal' }}>{it.name} {it.isMonthly && <span className="text-[8px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full ml-2">MENSAL</span>}</td><td className="p-3 text-center font-medium">{it.quantity}</td><td className="p-3 text-right text-gray-500">{(Number(it.price) || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td><td className="p-3 text-right font-bold text-gray-900">{(it.quantity * it.price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td></tr> ))}</tbody></table><div className="flex justify-end mt-8"><div className="w-1/2 space-y-2"><div className="flex justify-between items-center text-xs py-2 border-b border-gray-100"><span className="font-bold text-gray-400 uppercase text-[9px]">Investimento Total:</span> <span className="font-bold text-lg text-gray-900">{selectedProposal.totalOneTime.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div><div className="flex justify-between items-center text-xs py-2 bg-emerald-50/50 px-3 rounded text-emerald-800"><span className="font-bold uppercase text-[9px]">Recorrência Mensal:</span> <span className="font-bold text-lg">{selectedProposal.totalMonthly.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div></div></div></div><div className="grid grid-cols-2 gap-8 mb-16" style={{ letterSpacing: 'normal' }}><div className="bg-gray-50 p-6 rounded-lg text-xs border border-gray-100"><h3 className="font-bold mb-4 uppercase text-[9px] text-gray-400">Condições de Pagamento</h3><div className="space-y-2"><div className="flex items-center justify-between"><span className="font-medium text-gray-500">Forma:</span> <span className="font-bold uppercase">{selectedProposal.paymentMethod}</span></div><div className="flex items-center justify-between"><span className="font-medium text-gray-500">Parcelamento:</span> <span className="font-bold text-primary">{selectedProposal.installments}x de {(selectedProposal.totalOneTime / selectedProposal.installments).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div>{selectedProposal.firstAsDownPayment && selectedProposal.installments > 1 && ( <p className="text-[9px] font-bold text-primary text-right uppercase mt-1">(1ª Parcela como entrada)</p> )}</div></div>{selectedProposal.observations && ( <div className="bg-gray-50 p-6 rounded-lg text-xs border border-gray-100"><h3 className="font-bold mb-4 uppercase text-[9px] text-gray-400">Observações Adicionais</h3><p className="whitespace-pre-wrap italic text-gray-600 leading-relaxed text-[10px] text-left" style={{ letterSpacing: 'normal', wordSpacing: 'normal' }}>{selectedProposal.observations}</p></div> )}</div><div className="mt-auto pt-16 text-center"><div className="w-32 h-px bg-gray-200 mx-auto mb-4"></div><p className="text-[9px] text-gray-300 font-bold uppercase">{companyProfile.name}</p></div></div></ScrollArea> )}<DialogFooter className="print-hide border-t p-6 flex-wrap gap-2"><Button variant="outline" onClick={() => setSelectedProposal(null)}>Fechar</Button><Button variant="secondary" onClick={handleDownloadPdf} disabled={isDownloading}>{isDownloading ? <Loader2 className="animate-spin h-4 w-4" /> : <Download className="h-4 w-4" />} Baixar PDF</Button><Button variant="outline" className="gap-2" onClick={() => handleSendWhatsAppText(selectedProposal!)} title="Enviar resumo como texto"><Send className="h-4 w-4" /> WhatsApp (Texto)</Button><Button className="gap-2 bg-emerald-600 hover:bg-emerald-700" onClick={() => handleSharePdf(selectedProposal!)} disabled={isSharing}>{isSharing ? <Loader2 className="animate-spin h-4 w-4" /> : <Share2 className="h-4 w-4" />} Enviar PDF (WhatsApp)</Button></DialogFooter></DialogContent></Dialog>
