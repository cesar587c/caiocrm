  ...
  return (
    <div className="flex-1 space-y-6 p-8 pt-6">
      ...
      <Card className="mt-6"><CardHeader><CardTitle className="flex items-center gap-2"><ListChecks className="h-5 w-5"/>Propostas Geradas</CardTitle></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>ID</TableHead><TableHead>Cliente</TableHead><TableHead>Data</TableHead><TableHead>Investimento</TableHead><TableHead>Mensal</TableHead><TableHead className="text-right">Ações</TableHead></TableRow></TableHeader><TableBody>{proposals.length === 0 ? <TableRow><TableCell colSpan={6} className="text-center h-20 text-muted-foreground">Vazio</TableCell></TableRow> : proposals.map(p => ( <TableRow key={p.id}><TableCell className="font-bold">#{p.id}</TableCell><TableCell>{p.clientName}</TableCell><TableCell>{format(parseISO(p.proposalDate), 'dd/MM/yyyy')}</TableCell><TableCell className="text-primary font-medium">{p.totalOneTime.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</TableCell><TableCell className="text-emerald-500 font-medium">{p.totalMonthly.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</TableCell><TableCell className="text-right"><div className="flex justify-end gap-1"><Button variant="outline" size="icon" className="h-8 w-8 text-muted-foreground" onClick={() => handleCloneProposalClick(p)} title="Clonar Proposta"><Copy className="h-4 w-4" /></Button><Button variant="outline" size="icon" className="h-8 w-8" onClick={() => handleEditProposalClick(p)} title="Editar"><Pencil className="h-4 w-4" /></Button><Button variant="outline" size="icon" className="h-8 w-8" onClick={() => setSelectedProposal(p)} title="Imprimir"><Printer className="h-4 w-4" /></Button><Button variant="outline" size="icon" className="h-8 w-8 text-destructive" onClick={() => setDeletingProposal(p)} title="Excluir"><Trash2 className="h-4 w-4" /></Button></div></TableCell></TableRow> ))}</TableBody></Table></CardContent></Card>
        <Dialog open={!!selectedProposal} onOpenChange={(open) => !open && setSelectedProposal(null)}><DialogContent className="sm:max-w-5xl h-[90vh] flex flex-col"><DialogHeader className="print-hide"><DialogTitle>Pré-visualização da Proposta</DialogTitle></DialogHeader>{selectedProposal && ( <ScrollArea className="flex-1 -mx-6 px-6"><div id="proposal-preview" className="bg-white text-black p-16 shadow-lg max-w-4xl mx-auto my-8 border" style={{ fontFamily: "'Arial', sans-serif", letterSpacing: '0.015em' }}><div className="flex justify-between items-center mb-10" style={{ minHeight: '100px' }}><div className="flex items-center gap-8">{companyProfile.logoUrl && <img src={companyProfile.logoUrl} alt="Logo" className="max-h-20 w-auto" />}<div><h1 className="text-2xl font-bold uppercase tracking-tight leading-none" style={{ margin: '0' }}>{companyProfile.name}</h1></div></div><div className="text-right text-[11px] leading-relaxed text-gray-500 font-medium"><p>{companyProfile.address}</p><p>{companyProfile.email}</p><p className="text-black font-bold">{formatPhoneNumber(companyProfile.phone)}</p></div></div><div className="w-full h-px bg-gray-200 mb-12"></div><div className="text-center mb-16"><h2 className="text-3xl font-bold uppercase tracking-[0.2em] border-b-4 border-black pb-4 inline-block">Proposta Comercial</h2></div><div className="grid grid-cols-2 gap-12 mb-16 text-sm"><div className="space-y-1"><p className="font-bold text-gray-400 uppercase text-[10px] tracking-widest mb-2">Destinatário</p><p className="font-bold text-2xl leading-none mb-1">{selectedProposal.clientName}</p>{selectedProposal.contactName && ( <p className="text-base font-semibold text-primary uppercase tracking-tight">A/C: {selectedProposal.contactName}</p> )}<p className="text-gray-600 font-medium">{formatPhoneNumber(selectedProposal.clientPhone || '')}</p></div><div className="text-right flex flex-col justify-end space-y-2"><p className="text-gray-400 uppercase text-[10px] tracking-widest">Informações Gerais</p><p className="leading-none text-base"><span className="font-bold">Proposta:</span> #{selectedProposal.id}</p><p className="leading-none text-base"><span className="font-bold">Emissão:</span> {format(parseISO(selectedProposal.proposalDate), 'dd/MM/yyyy')}</p><p className="leading-none text-base text-destructive font-bold"><span className="text-black">Validade:</span> {format(parseISO(selectedProposal.validityDate), 'dd/MM/yyyy')}</p></div></div><div className="space-y-6 text-sm mb-16 leading-relaxed text-justify text-gray-800"><p>Com mais de 18 anos de experiência, somos a junção de Soluções especializada em tecnologia.</p><p>Na área de Consultoria da Tecnologia dispomos das mais modernas ferramentas e profissionais altamente qualificados.</p><p>A {companyProfile.name} conta hoje com a parceria de grandes empresas para atender seus clientes de forma ágil e com grande qualidade profissional no mínimo de tempo e passa por rigor de análise em vários critérios, a começar pelo atendimento ao cliente.</p></div><div className="mb-16"><table className="w-full text-left text-sm border-collapse"><thead className="bg-gray-50"><tr><th className="p-4 font-bold border-b-2 border-gray-900 uppercase text-[11px] tracking-wider">Item / Descrição</th><th className="p-4 text-center font-bold border-b-2 border-gray-900 uppercase text-[11px] tracking-wider">Qtd.</th><th className="p-4 text-right font-bold border-b-2 border-gray-900 uppercase text-[11px] tracking-wider">Preço Unit.</th><th className="p-4 text-right font-bold border-b-2 border-gray-900 uppercase text-[11px] tracking-wider">Subtotal</th></tr></thead><tbody>{selectedProposal.items.map((it, i) => ( <tr key={i} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors"><td className="p-4 font-medium">{it.name} {it.isMonthly && <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded ml-2">MENSAL</span>}</td><td className="p-4 text-center">{it.quantity}</td><td className="p-4 text-right text-gray-600">{(Number(it.price) || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td><td className="p-4 text-right font-bold">{(it.quantity * it.price).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</td></tr> ))}</tbody></table><div className="flex justify-end mt-8"><div className="w-1/2 space-y-3"><div className="flex justify-between items-center text-sm py-2 border-b border-gray-100"><span className="font-bold text-gray-500 uppercase tracking-widest text-[10px]">Total Investimento:</span> <span className="font-bold text-2xl">{selectedProposal.totalOneTime.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div><div className="flex justify-between items-center text-sm py-2 text-emerald-700"><span className="font-bold uppercase tracking-widest text-[10px]">Total Recorrência:</span> <span className="font-bold text-2xl">{selectedProposal.totalMonthly.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div></div></div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-20"><div className="bg-gray-50/50 p-8 rounded-2xl text-sm border border-gray-100 h-full"><h3 className="font-bold mb-6 uppercase text-[10px] text-gray-400 tracking-[0.2em]">Condições de Pagamento</h3><div className="space-y-4"><p className="flex items-baseline justify-between"><span className="font-bold text-gray-500 text-[10px] uppercase">Forma:</span> <span className="font-bold uppercase text-base">{selectedProposal.paymentMethod}</span></p><div className="flex flex-col gap-1"><div className="flex items-baseline justify-between"><span className="font-bold text-gray-500 text-[10px] uppercase">Parcelamento:</span> <span className="font-bold text-base">{selectedProposal.installments}x de {(selectedProposal.totalOneTime / selectedProposal.installments).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</span></div>{selectedProposal.firstAsDownPayment && selectedProposal.installments > 1 && ( <p className="text-[10px] font-bold text-primary text-right uppercase tracking-tighter">(Sendo a 1ª como entrada)</p> )}</div></div></div>{selectedProposal.observations && ( <div className="bg-gray-50/50 p-8 rounded-2xl text-sm border border-gray-100 h-full"><h3 className="font-bold mb-6 uppercase text-[10px] text-gray-400 tracking-[0.2em]">Observações Gerais</h3><p className="whitespace-pre-wrap italic text-gray-700 leading-relaxed text-xs text-justify">{selectedProposal.observations}</p></div> )}</div><div className="mt-32 text-center"><div className="w-64 h-px bg-gray-300 mx-auto mb-4"></div><p className="text-[12px] text-gray-400 font-bold uppercase tracking-[0.3em]">{companyProfile.name}</p></div></div></ScrollArea> )}<DialogFooter className="print-hide border-t pt-4 flex-wrap gap-2"><Button variant="outline" onClick={() => setSelectedProposal(null)}>Fechar</Button><Button variant="secondary" onClick={handleDownloadPdf} disabled={isDownloading}>{isDownloading ? <Loader2 className="animate-spin h-4 w-4" /> : <Download className="h-4 w-4" />} PDF</Button><Button variant="outline" className="gap-2" onClick={() => handleSendWhatsAppText(selectedProposal!)} title="Enviar resumo como texto"><Send className="h-4 w-4" /> WhatsApp (Texto)</Button><Button className="gap-2 bg-emerald-600 hover:bg-emerald-700" onClick={() => handleSharePdf(selectedProposal!)} disabled={isSharing}>{isSharing ? <Loader2 className="animate-spin h-4 w-4" /> : <Share2 className="h-4 w-4" />} Enviar PDF (WhatsApp)</Button></DialogFooter></DialogContent></Dialog>
